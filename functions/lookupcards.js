    ...
    const idQueries = firstFiveNumbers.map(id => faunadb.query.Get(faunadb.query.Match(faunadb.query.Index("cards_by_id"), id)));
       ...
       const result = await client.query(
        faunadb.query.Map(
          ...
          faunadb.query.Union(...[idQueries]), // <-- wrap the 'idQueries' array in square brackets
      ...
      // Initialize a variable called 'allvalues' to 0
      let allvalues = 0;
  
      // Iterate over the 'data' array
      data.forEach(doc => {
        // Calculate the 'individualvalue' of each document by adding up the values in the 'scores' object
        doc.individualvalue = Object.values(doc.scores).reduce((sum, score) => sum + score, 0);
